...
import { SonaDjMode, getSonaDjMode } from '@/store/sona-dj.store'
import {
  getSessionGenreSet,
...
export function createPlaybackEngine(store: PlaybackEngineStoreApi) {
  ...
  const applySessionModeFilter = (songs: ISong[]) => {
    ...
    return songs.filter((song) =>
      matchesSessionModeGenre(song, mode, settings.focusGenres, settings.nightGenres),
    )
  }

  const clearInjectedSongs = () => {
    store.setState((state) => {
      state.songlist.currentList = state.songlist.currentList.map(stripInjectedSong)
      state.songlist.originalList = state.songlist.originalList.map(stripInjectedSong)
    })
  }
  ...
  const queueSonaDjTrack = (mode: SonaDjMode, songToInject: ISong) => {
    ...
    const newCurrentList = addNextSongList(currentSongIndex, currentList, [injectedSong])
    const indexOnOriginalList = originalList.findIndex(
      (song) => song.id === currentSong.id,
    )
  ...
  const getStrictSessionCandidates = async (mode: SessionMode) => {
    const settings = store.getState().settings.sessionMode
    const allowedGenres = getSessionGenreSet(mode, settings.focusGenres, settings.nightGenres)
  ...
  const getRuntimeRandomSong = async (currentSong?: ISong, excludedIds?: Set<string>) => {
    ...
    const eligibleRandomSongs = applySessionModeFilter(randomSongs ?? []).filter(
      (song) => song.id !== currentSong?.id && !excludedIds?.has(song.id),
    )
  ...
  const ensureRuntimeShuffleNextTrack = async () => {
    ...
    try {
      ...
      if (appendedSongs.length === 0) {
        if (strictSessionFilter) appendedSongs.push(activeSong)
        else return
      }

      const newCurrentList = addNextSongList(currentSongIndex, currentList, appendedSongs)
      const indexOnOriginalList = originalList.findIndex(
        (song) => song.id === currentSong.id,
      )
      const newOriginalList =
  ...
  const startRuntimeShuffleAll = async () => {
    ...
    const { currentSong, currentList, currentSongIndex } = store.getState().songlist
    const activeSong = currentSong ?? currentList[currentSongIndex]
    const sessionMode = getSessionMode()

    let firstSong = await getRuntimeRandomSong()
    if (!firstSong && sessionMode !== 'off' && activeSong?.id) {
      if (matchesSessionModeGenre(activeSong, sessionMode)) firstSong = activeSong
    }
    ...
    const secondSong = await getRuntimeRandomSong(firstSong, new Set([firstSong.id]))
